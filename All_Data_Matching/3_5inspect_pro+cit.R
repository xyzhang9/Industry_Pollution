...
for (year in years) {
  ...
  for (i in 1:nrow(selected_cols)) {
    ...
    if (sdxm_individual != "NA"&&sdxm_individual != "wrong sdxm") {
      # 提取sdxm_individual的前4位数字（省）
      prefix <- substr(sdxm_individual, 1, 2)
      # 后面补0
      suffix_padded <- "0000"
      # 合并前缀和后缀
      prefix_padded <- paste0(prefix, suffix_padded)
      # 构建匹配条件
      matching_rows <- which(substr(sheet1$代码, 1, 6) == prefix_padded)
      
      # 如果没有匹配到任何行，将zb设为1，否则获取匹配的行内容的B列值
      if (length(matching_rows) == 0) {
        zb <- 1
        reference_sheng<-"nonfound"
      } else {
        reference_sheng <- sheet1$省市[matching_rows]
        
        # 判断reference是否是information中某个值的子字符串
        is_duplicate <- any(grepl(reference_sheng, information))
        is_duplicate <- any(grepl(paste(substr(reference_sheng, 1, 2), collapse = "|"), information))
        
        if (!is_duplicate) {
          last_char_reference <- substr(reference_sheng, nchar(reference_sheng), nchar(reference_sheng))
          is_duplicate2 <- any(grepl(last_char_reference, information2))
          if (!is_duplicate2) {
            zb <- 3
            reference_sheng <- reference_sheng
          }else{
            zb <- 2
          }
        }else{
          last_char_reference <- substr(reference_sheng, nchar(reference_sheng), nchar(reference_sheng))
          sheng_last <- last_char_reference
        }
      }
      # 提取sdxm_individual的前2位数字（市）
      prefix <- substr(sdxm_individual, 1, 4)
      # 后面补0
      suffix_padded <- "00"
      # 合并前缀和后缀
      prefix_padded <- paste0(prefix, suffix_padded)
      # 构建匹配条件
      if(zb==0 & sheng_last=="市"){
        matching_rows <- which(sheet1$代码 == sdxm_individual)
      }else if(zb==0 & sheng_last=="省"){
        matching_rows <- which(substr(sheet1$代码, 1, 6) == prefix_padded)
        if (length(matching_rows) == 0){
          matching_rows <- which(sheet1$代码 == sdxm_individual)
          if(length(matching_rows) != 0){
            reference_shi <- sheet1$省市[matching_rows]
            last_char_reference <- substr(reference_shi, nchar(reference_shi), nchar(reference_shi))
            if(last_char_reference != "市"){
              matching_rows <- which(substr(sheet1$代码, 1, 6) == prefix_padded) 
            }
          }
        }
      ...
      if (length(matching_rows) == 0) {
        zb <- 1
        reference_shi<-"nonfound"
      } else {
        reference_shi <- sheet1$省市[matching_rows]
        
        # 判断reference是否是information中某个值的子字符串
        is_duplicate <- any(grepl(reference_shi, information))
        is_duplicate <- any(grepl(paste(substr(reference_shi, 1, 2), collapse = "|"), information))
    ...
    reference <- paste(reference_sheng, reference_shi)
    if (zb == 0){
    ...
    if (yzbm_individual != "NA"&&yzbm_individual != "wrong yzbm"){
      ...
      if(nchar(yzbm_individual)==5){
        yzbm_individual <- paste0("0", yzbm_individual)
      }
      ...
      if (length(matching_rows) == 0) {
        ...
        for(j in matching_rows){
          ...
          reference3 <- sheet2$C[j]
          reference_sheng <- reference2
          
          last_char_reference2 <- substr(reference2, nchar(reference2), nchar(reference2))
          reference_save<- c()
         
          matching_rows2 <- which(sheet2$C == reference3)
          for(i in matching_rows2){
            reference1 <- sheet2$B[i]
            
            if(last_char_reference2=="市"){
              reference_save <-  c(reference_save, reference1)
            }else{
              #验证是否对应两个市
          ...
          for(k in reference_save){
            reference1 <- k
            is_duplicate1 <- any(grepl(reference1, information))#城市级是否匹配
            if(is_duplicate1){
              zbb <- 0
              reference_shi<- reference1
            }else if(zbb != 0 & !is_duplicate1 ){
            # 判断reference2的最后一个字符是否包含在information里
            last_char_reference <- substr(reference1, nchar(reference1), nchar(reference1))
            is_duplicate2 <- any(grepl(last_char_reference, information2))
            if (is_duplicate2) {
              zbb <- 2
    ...
    if (行政区代码_individual != "NA"&&行政区代码_individual != "wrong 行政区代码") {
      # 提取sdxm_individual的前4位数字（省）
      prefix <- substr(行政区代码_individual, 1, 2)
      # 后面补0
      suffix_padded <- "0000"
      # 合并前缀和后缀
      prefix_padded <- paste0(prefix, suffix_padded)
      ...
      # 提取sdxm_individual的前2位数字（市）
      prefix <- substr(行政区代码_individual, 1, 4)
      # 后面补0
      suffix_padded <- "00"
      # 合并前缀和后缀
      prefix_padded <- paste0(prefix, suffix_padded)
