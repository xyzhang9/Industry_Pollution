...
output_folder <- "D:/照片材料/文件/应聘科研助理_炊晨阳_武汉大学_18028377912/环境/数据/计算3/地址/all_pro/"
    ...
    if (yzbm_individual != "NA"&&yzbm_individual != "wrong yzbm"){
      ...
      if (length(matching_rows) == 0) {
        ...
        for(j in matching_rows){
          ...
          reference_save <- sheet2$A[matching_rows[1]]
          #验证是否对应两个省
          
          if (reference2 != reference_save) {
            stop("Error: reference2 和 reference_save 不相等，程序停止运行。\n", 
                 "年份：", year, "\n", 
                 "yzbm_individual 值：", yzbm_individual)
          }
          
          reference <- paste(reference1, reference2, sep = " ")
          
          # 判断reference是否是information中某个值的子字符串
          is_duplicate1 <- any(grepl(reference2, information))#城市级是否匹配
          
          if (is_duplicate1) {
            zbb <- 0
            row_new$yzbm_reference<- reference2
          }
          }
        }
      if(zbb==0){
        ...
        last_char_reference <- substr(reference_save, nchar(reference_save), nchar(reference_save))
        is_duplicate2 <- any(grepl(last_char_reference, information))
        if (!is_duplicate2) {
          zb <- 3
          row_new$yzbm_reference<- reference2
        }else{
